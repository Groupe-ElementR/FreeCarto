...
#' \itemize{
#' \item{\code{\link{layoutLayer}} to make a nice frame around a map}
#' \item{\code{\link{staticLayer}} to add layers on a map}
#' \item{\code{\link{labelLayer}} to add labels on a map}
#' }
#' @docType package
NULL
...
#' @name TNdeleg
#' @description This dataset contains some socio-economic data on tunisian delegations in 2004 (population, active population, unemployed population, housing, households).
#' @usage data(TNdeleg)
...
#' @details This data.frame can be joined to the \code{\link{TNdeleg.spdf}} SpatialPolygonsDataframe. The \code{del_code} field from TNDeleg matches the \code{del_code} field from TNdeleg.spdf.
#' @source Institut National de la Statistique (INS). General census of population and housing 2004 (last update 18-06-2013). Data gathered on October 2014.
...
#' @details This SpatialPolygonsDataFrame can be joined to the \code{\link{TNdeleg}} data.frame.
#' The \code{del_code} field from TNDeleg matches the \code{del_code} field from TNdeleg.spdf.
...
#' metropolitan departments from 1975 to 2011 (population, age structure, employement, sector of employement).
#' @usage data(FRdep)
#' @format This data.frame has 96 rows and 80 columns.
#' @details This data.frame can be joined to the \code{\link{FRdep.spdf}} SpatialPolygonsDataframe. The \code{dep_code} field from FRdep matches the \code{dep_code} field from FRdep.spdf.
#' "\code{****}" in field names refers to the census years (1975, 1982, 1990, 1999, 2006, 2011).
#' @field dep_code department code (character)
#' @field dep_name department name (character)
#' @field pXXXX_**** Population (numeric). "\code{XXXX}" refers to the different age groups.
#' \itemize{
...
#' @field farm_**** 25-54 years old population employed in the farming sector (numeric).
#' @field indu_**** 25-54 years old population employed in the industrial sector (numeric).
#' @field cons_**** 25-54 years old population employed in the construction sector (numeric).
#' @field serv_**** 25-54 years old population employed in the service sector (numeric).
#' @source Institut national de la statistique et des études économiques (INSEE). General census of population 1975, 1982, 1990, 1999, 2006, 2011. Data gathered on November 2014.
...
#' @field id Unique nuts id (character)
#' @field emp2008 Active population in employment in 2008 (thousands persons) (numeric)
#' @field act2008 Active population in 2008 (thousands persons) (numeric)
#' @field unemp2008 Active population unemployed in 2008 (thousands persons) (numeric)
#' @field birth_2008 Number of birth in 2008 (live birth) (numeric)
#' @field death_2008 Number of death in 2008 (death) (numeric)
#' @field gdppps1999 Gross domestic product (Purchasing Power Standards) in 1999 (million euros) (numeric)
#' @field gdppps2008 Gross domestic product (Purchasing Power Standards) in 2008 (million euros) (numeric)

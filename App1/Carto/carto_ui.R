output$carto <- renderUI({
  fluidRow(
    column(3, wellPanel(
      radioButtons(inputId = "mapType", label = "Type de Carte",
      ...
      conditionalPanel(
      condition = "input.mapType == 'symbols'",

      selectInput(inputId = "var2", label = "Variable",
                                    selectize = TRUE, multiple = FALSE,
                                    choices = colnames(isolate(baseData$data)),
                                    selected = colnames(isolate(baseData$data)))
 ...
 column(9,plotOutput("map", height="700"))
  )
})
